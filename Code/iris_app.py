...
model1= open("rfc.pkl", "rb")
knn_clf=joblib.load(model)
rfc_clf=joblib.load(model1)

model2 = keras.models.load_model('flower.h5')
...
if st.button("Click Here to Classify"):
	...
	st.header('Random Forrest Classifier')
	prediction1 = rfc_clf.predict(input_variables)
	st.image(setosa) if prediction1 == 0 else st.image(versicolor)  if prediction1 == 1 else st.image(virginica) 
